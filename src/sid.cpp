...
void Sid::set_chip_model(Model model) {
    impl->sid.set_chip_model(model == Model::MOS6581 ? reSID::MOS6581 : reSID::MOS8580);
}
void Sid::set_sampling_method(SamplingMethod sampling_method) {
    impl->sid.set_sampling_parameters(CLOCKRATE_PAL, reSID::sampling_method(sampling_method), MIXRATE);
}
...
std::array<float, 3> Sid::get_env_levels() {
    reSID::SID::State state = impl->sid.read_state();
